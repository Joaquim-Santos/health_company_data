...
class BaseRepository(ABC):
    ...
    def _get_model(self):
        ...
        """
        model_class = getattr(
            importlib.import_module(self.model_module), self.model_class
        )
        return model_class()
    ...
    def find(self, id_=None, filters=None):
        ...
        if not model:
            raise EntityNotFound(
                f"Entidade {self._get_model().__tablename__} não encontrada."
            )
    ...
    def find_many(self, filters, join_query=None):
        ...
        if join_query is None:
            ...
            for entity in join_query.filter(filters).all():
                ...
                item = {
                    key: Serializer.json_serialize(value)
    ...
    def create_many(self, entities, commit_at_the_end=True):
        ...
        for entity in entities:
            new_entities.append(self.create(entity, commit_at_the_end=False))
    ...
    def update(self, id_, attributes, commit_at_the_end=True, filters=None):
        ...
        if not model:
            raise EntityNotFound(
                f"Entidade {self._get_model().__tablename__} a ser atualizada não encontrada."
            )
    ...
    def delete(self, id_, commit_at_the_end=True):
        ...
        if not model:
            raise EntityNotFound(
                f"Entidade {self._get_model().__tablename__} a ser apagada não encontrada."
            )
    ...
    def delete_by_filter(self, filters, commit_at_the_end=True):
        ...
        """
        db.session.query(self._get_model().__class__).filter(filters).delete()
