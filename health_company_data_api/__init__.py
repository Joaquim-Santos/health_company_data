...
builder.add_resources(
    ...
    router_file_path="health_company_data_api/configurations/routes.json",
)
...
def log_request_info():
    logger.log.info(
        f"Acesso ao endpoint: {request.method} {request.url}\n"
        f"Headers:\n{request.headers}"
...
def after_request(response):
    logger.log.info(
        f"Resposta com sucesso do endpoint: {request.method} {request.url}\n"
        f"data:\n{response.json}\n"
    )

    response.headers.add("Access-Control-Allow-Origin", "*")
    response.headers.add(
        "Access-Control-Allow-Headers", "Content-Type,Authorization,chave-petrobras"
    )
