...
class PatientsFiltersSchema(Schema):
    first_name = fields.Str(
        required=False, validate=[validate.Length(min=3, max=30)], allow_none=True
    )
    last_name = fields.Str(
        required=False, validate=[validate.Length(min=3, max=30)], allow_none=True
    )
    start_age = fields.Int(
        required=False, validate=[validate.Range(min=18)], allow_none=True
    )
    end_age = fields.Int(
        required=False, validate=[validate.Range(min=18)], allow_none=True
    )
    ...
    def post_load(self, data, many, **kwargs):
        ...
        try:
            if data_without_none["start_age"] > data_without_none["end_age"]:
                raise ValidationError(
                    "Idade inicial não pode ser maior do que a final."
                )
        except KeyError:
            pass
        ...
        for key in ["start_age", "end_age"]:
            try:
                data_without_none[key] = datetime.now() - relativedelta(
                    years=data_without_none[key]
                )
                data_without_none[key] = data_without_none[key].replace(
