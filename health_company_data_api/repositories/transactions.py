...
class TransactionsRepository(AbstractRepository):
    ...
    def get_transactions(self, transactions_filters: dict) -> list:
        join_query = (
            TransactionsModel.query.join(
                PatientsModel, TransactionsModel.patient_uuid == PatientsModel.uuid
            )
            .join(
                PharmaciesModel, TransactionsModel.pharmacy_uuid == PharmaciesModel.uuid
            )
            .add_columns(
                PatientsModel.uuid.label("patient_id"),
                PatientsModel.first_name,
                PatientsModel.last_name,
                PatientsModel.date_of_birth,
                PharmaciesModel.uuid.label("pharmacy_id"),
                PharmaciesModel.name,
                PharmaciesModel.city,
                TransactionsModel.uuid.label("transaction_id"),
